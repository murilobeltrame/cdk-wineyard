...
interface WineriesApiProps extends NestedStackProps {
    cache: {
...
export class WineriesApiStack extends NestedStack {
    api: RestApi

    constructor(scope: Construct, id: string, props: WineriesApiProps) {
        super(scope, id, props);
        
        const entityName = 'Wineries'
        ...
        const propsFor = (fileName: string): NodejsFunctionProps => {
            return {
                entry: path.join(__dirname, `../functions/wineries/${fileName}.ts`),
                handler: 'handler',
